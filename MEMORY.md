...
- 2026-05-22: Dan a corectat regula de produs: nu vrea dovezi/screenshot-uri pentru fiecare workflow; screenshot-ul este cerut doar când workflow-ul este vizual sau când claim-ul/gate-ul depinde de dovadă vizuală. Phone Network `3.9.30` este LIVE GO pentru endpointul produs Reddit health scan după OTA agent Android `4.0.24`: server live `3.9.30` / `d0068afc25d2e47121a3d0695c69e456b8a94074`, OTA push pe device `d35b34cb-b2ee-4f6e-a8c6-a72cca14a0dd` a trecut de la agent `4.0.23` la `4.0.24`, iar `POST /api/agency/reddit/account-health-scans` a completat run `0a3dac79-f064-4ac5-9b0b-d5ab466e5385`, task `d38aba96-5b30-4715-ba12-be7426444d59`, workflow `a593d88b-17c7-4622-8653-6a2e6a7dba99`, cacheKey `3aa337af7a480a0237d514b8`. Output business real: `loggedIn=true`, `homeFeedVisible=true`, `searchSurfaceAvailable=true`, `screenState=reddit_home_feed`, `challengeDetected=false`, `loginWallDetected=false`, 0 token usage, 0 LLM/VLM.
...
**ENDPOINTS:**
- Qwen live: `http://enkzoned.go.ro:12321/v1/chat/completions` (port 8000 intern), served model `qwen3.6-27b`; din 2026-05-23 rulează pentru test `lyf/Qwen3.6-27B-uncensored-heretic-v2-NVFP4-MTP` în containerul Docker `qwen36` pe vLLM `0.17.1-cu130`. Răspunsurile pot include `message.reasoning` separat de `message.content`.
